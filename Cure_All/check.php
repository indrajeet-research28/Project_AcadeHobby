...
if(isset($_POST['drug_name']))
{
	...
	$sql3="INSERT INTO tempPrescri(customer_id,customer_name,age,sex,postal_address,phone,drug_name,strength,dose,quantity)
	...
	$sql=mysqli_query($con,$sql3);
						
						$_SESSION['quantity']=$quantity;
						
	
	$sql1="SELECT cost FROM stock WHERE drug_name='{$drug}'";					
						
	$get_cost=mysqli_query($con,$sql1);
...
$getDetails=mysqli_query("SELECT invoice,drug,cost,quantity FROM invoice_details WHERE invoice='{$invoice}'");
$getQuantity=mysqli_query("SELECT quantity FROM invoice_details WHERE invoice_id='{$invoice}'");
...
while($item5=mysqli_fetch_array($getDetails))
			{
			$getDrug=mysqli_query("SELECT drug_name FROM stock WHERE stock_id='{$item5['drug']}'");
