...
$sql="SELECT customer_id FROM prescription WHERE invoice_id='{$invNo}' ";
$getCid=mysqli_query($con,$sql);

$sql1="SELECT customer_name FROM invoice WHERE invoice_id='{$invNo}' ";
$getName=mysqli_query($con,$sql1);


$details1=mysqli_fetch_array($getName); 
$details=mysqli_fetch_array($getCid);

	$sql2="INSERT INTO receipts(reciptNo,customer_id,total,payType,serialno,served_by)
				VALUES('{$recNo}','{$details['customer_id']}','{$amount}','{$pType}','{$serial}','{$_SESSION['username']}')  ";
	
	$sqlP=mysqli_query($con,$sql2);
...
$titling= <<<EOD
<strong> <font style="font-size:11">Cure All</font> </strong> <br> <strong>Official Receipt</strong><br>
Student Center Ground Floor,<br> P.O. Channasandra, Bangalore <br> Tel: +91 8147857847 <br> E-mail: cureall@yahoo.com 
<br>-----------------------------------------
EOD;
$header = array('Drug','Price.', 'Qtty' ,'Cost(Rs.)');
$ddt=<<<EOD
$time   
EOD;
$html = <<<EOD
